...
try:
    from scripts.prepare_nurec_obstacles import load_sequence_tracks, load_obstacle_rows, build_queries, load_time_record, _dump, _csv, pose_yaw, _pose, _wrap
except ModuleNotFoundError:
    from prepare_nurec_obstacles import load_sequence_tracks, load_obstacle_rows, build_queries, load_time_record, _dump, _csv, pose_yaw, _pose, _wrap
...
def _crosscheck(seq, obs):
    ...
    for s in seq:
        ...
        if not o: continue
        center=math.dist(s["center"],o["center"]); yaw=abs(_wrap(pose_yaw(_pose(s["center"],s["quaternion"]))-pose_yaw(_pose(o["center"],o["quaternion"]))))
...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--manifest",required=True); p.add_argument("--time-alignment-jsonl",required=True); p.add_argument("--output-root",required=True); a=p.parse_args(); root=Path(a.output_root); root.mkdir(parents=True,exist_ok=True); time_path=Path(a.time_alignment_jsonl); manifest=[json.loads(x) for x in Path(a.manifest).read_text(encoding="utf-8").splitlines() if x.strip()]; summaries=[]; cross_rows=[]; cf_rows_all=[]; ttc_rows_all=[]
    ...
    for item in manifest:
        ...
        cf_rows=[{"clip_id":cid,"timestamp_us":q,"temporal_object_evidence":min((abs(t-q) for t in obs_ts),default=10**18)<=50_000,"frame_completeness_status":"UNRESOLVED"} for q in cf]; ttc_rows=[{"clip_id":cid,"timestamp_us":q,"temporal_object_evidence":min((abs(t-q) for t in obs_ts),default=10**18)<=100_000,"frame_completeness_status":"UNRESOLVED"} for q in ttc]; cf_rows_all.extend(cf_rows); ttc_rows_all.extend(ttc_rows); _csv(out/"cf_observation_completeness.csv",cf_rows); _csv(out/"ttc_observation_completeness.csv",ttc_rows); _csv(out/"obstacle_crosscheck.csv",pairs)
    ...
    _dump(root/"obstacle_provenance_summary.json",{"clip_count":len(summaries),"clips":summaries,"sequence_tracks_pose_frame":"STRONGLY_SUPPORTED_NOT_VERIFIED","obstacle_transform_status":"PARTIALLY_VERIFIED","normalized_obstacle_status":"PARTIALLY_VERIFIED","empty_scene_attestation_status":"UNRESOLVED_EMPTY_ATTESTATION","complete_observation_timeline_status":"UNRESOLVED","cf_data_ready":False,"ttc_data_ready":False})
    _dump(root/"pai_obstacle_schema.json",{"status":"NOT_AVAILABLE_IN_LOCAL_HF_CACHE","official_source_available_clips":[],"expected_fields":["timestamp","center","size","orientation","track_id","class","reference_frame_id","reference_frame_timestamp_us"],"evidence_origin":"LOCAL_AUTOMATED_INSPECTION"})
    _dump(root/"pai_obstacle_frame_contract.json",{"status":"UNRESOLVED_EXTERNAL_SOURCE_UNAVAILABLE","source":"official PhysicalAI obstacle.offline","verified":False,"evidence_origin":"LOCAL_AUTOMATED_INSPECTION"})
    _dump(root/"pai_ncore_nurec_bridge.json",{"status":"PUBLIC_CONTRACT_VERIFIED","pai_to_ncore":{"source":PUBLIC_NCORE,"verified":True,"fields":["timestamp_us","reference_frame_id","reference_frame_timestamp_us","bbox3"]},"ncore_to_nurec":{"source":PUBLIC_INSTANT,"verified":True,"operation":"reference_frame_to_world via pose_graph.evaluate_poses and T_world_world_base"},"api":PUBLIC_API,"local_obstacle_source_available":False})
    _dump(root/"local_sequence_tracks_version_evidence.json",{"clips":{s["clip_id"]:{"version":s.get("local_pipeline_version"),"status":s.get("pipeline_compatibility_status")} for s in summaries},"status":"STRONGLY_SUPPORTED_NOT_EXACTLY_PINNED"})
    _dump(root/"sequence_tracks_serialization_contract.json",{"status":"STRONGLY_SUPPORTED_NOT_EXACTLY_PINNED","fields":{"tracks_id":"tracks_data.tracks_id","tracks_poses":"tracks_data.tracks_poses","tracks_timestamps_us":"tracks_data.tracks_timestamps_us","tracks_label_class":"tracks_data.tracks_label_class","cuboids_dims":"cuboidtracks_data.cuboids_dims"}})
    _dump(root/"track_and_rig_common_frame_contract.json",{"public_status":"STRONGLY_SUPPORTED_NOT_VERIFIED","local_status":"STRONGLY_SUPPORTED_NOT_VERIFIED","verified":False})
    _dump(root/"obstacle_dimension_semantics.json",{"status":"LENGTH_WIDTH_HEIGHT_PUBLIC_DOC_ONLY","local_output_names":"extent_x,extent_y,extent_z","verified":False})
    _dump(root/"obstacle_orientation_semantics.json",{"quaternion_order":"qx,qy,qz,qw","verified":True,"source":PUBLIC_NCORE})
    _dump(root/"observation_completeness_sources.json",{"status":"UNRESOLVED","sensor_and_pose_ranges_are_not_annotation_completeness":True,"explicit_empty_attestation_found":False})
    _dump(root/"empty_scene_attestation_audit.json",{"status":"UNRESOLVED_EMPTY_ATTESTATION","reason":"object-only rows and sensor/pose ranges do not prove complete cuboid annotation coverage"})
    _dump(root/"obstacle_frame_final_summary.json",{"clip_count":len(summaries),"pai_obstacle_available_clips":[],"obstacle_transform_status":"PARTIALLY_VERIFIED","normalized_obstacle_status":"PARTIALLY_VERIFIED","geometry_block_ready_to_close":False,"remaining_blockers":["OFFICIAL_PAI_OBSTACLE_SOURCE_UNAVAILABLE","LOCAL_SEQUENCE_TRACKS_SERIALIZER_NOT_EXACTLY_PINNED","TRACK_RIG_COMMON_FRAME_NOT_LOCALLY_VERIFIED","EMPTY_SCENE_ATTESTATION_UNRESOLVED"]})
    _csv(root/"five_clip_obstacle_crosscheck.csv",cross_rows); _csv(root/"five_clip_pai_nurec_obstacle_crosscheck.csv",cross_rows); _csv(root/"cf_observation_completeness.csv",cf_rows_all); _csv(root/"ttc_observation_completeness.csv",ttc_rows_all)
