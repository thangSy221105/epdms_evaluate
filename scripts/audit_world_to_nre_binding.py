...
def main():
    ...
    for item in manifest:
        ...
        first=raw[0][1]; rebase=inv(first); rebased_first=_mm(rebase,first); invw=inv(matrix); pairs_a=[]; pairs_b=[]
        for ts,p in raw:
            local_pose=_mm(rebase,p); target=interpolate_pose(local,ts+rec["offset_us"]); pairs_a.append((_mm(matrix,local_pose),target)); pairs_b.append((_mm(invw,local_pose),target))
        ...
        ego_contract.append({"clip_id":cid,"world_to_nre_found":True,"world_to_nre_matrix":matrix,"source":"rig_trajectories.json:world_to_nre.matrix","direction_candidate":"NCore world -> local NuRec scene","public_semantics":NCORE_FRAME_CONVERSION,"T_first_raw":first,"T_rebase":rebase,"rebased_first_identity":all(abs(rebased_first[i][j]-(1.0 if i==j else 0.0))<1e-9 for i in range(4) for j in range(4)),"A_forward_metrics":ma,"B_inverse_metrics":mb,"selected_direction":direction})
    ...
    _dump(out/"world_to_nre_binding_final_summary.json",{"pai_ncore_rebase_applied":True,"rebased_first_pose_identity":all(x["rebased_first_identity"] for x in ego_contract),"world_to_nre_found":True,"world_to_nre_direction":"NCore world -> local NuRec scene","world_to_nre_binding_status":"VERIFIED" if verified else "PARTIALLY_VERIFIED","ego_matched_timestamp_count":sum(x["matched_timestamp_count"] for x in forward),"max_ego_center_rmse_m":max(x["center_rmse_m"] for x in forward),"max_ego_center_p95_m":max(x["center_p95_m"] for x in forward),"max_ego_center_max_m":max(x["center_max_m"] for x in forward),"max_ego_yaw_rmse_deg":max(x["yaw_rmse_deg"] for x in forward),"max_ego_yaw_p95_deg":max(x["yaw_p95_deg"] for x in forward),"max_ego_yaw_max_deg":max(x["yaw_max_deg"] for x in forward),"final_obstacle_transform_formula":"world_to_nre @ T_rig_world_local(t) @ T_object_rig","matched_observation_count":0,"obstacle_stage_ran":False,"obstacle_transform_status":"BLOCKED_BY_EGO_BINDING","normalized_obstacle_status":"BLOCKED_BY_EGO_BINDING","obstacle_geometry_block_ready_to_close":False,"per_clip_offset_rederived":False,"cf_proxy_label_set_ready":True,"ttc_proxy_label_set_ready":True,"cf_data_ready":False,"ttc_data_ready":False,"physical_world_obstacle_completeness":"NOT_CLAIMED","remaining_blockers":["EGO_WORLD_TO_NRE_BINDING_NOT_VERIFIED" if not verified else "OBSTACLE_STAGE_NOT_IMPLEMENTED"],"recommended_next_step":"do not promote obstacle transform until ego binding passes provenance and numerical acceptance"})
