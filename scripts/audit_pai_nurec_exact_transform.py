...
def local_world_ref(clip):
    ...
    return {"exact_field_present": False, "candidate_world_to_nre": d.get("world_to_nre"), "T_world_base_present": "T_world_base" in d,
            "status": "UNRESOLVED_LOCAL_SERIALIZATION_EDGE", "reason": "local files expose world_to_nre but do not declare it as Instant-NuRec get_frames_T_source_sensor T_world_ref"}
...
def compare(clip, pai, ego, local, offset, world_ref):
    ...
    for r in pai:
        ...
        wr = _pose([float(x) for x in world_ref["candidate_world_to_nre"]["matrix"][0:3][3:4]] if False else [0,0,0],[0,0,0,1]) if False else _pose([0,0,0],[0,0,0,1])
        # The candidate matrix is deliberately not applied: its semantic edge is unproven.
...
def main():
    ...
    for item in manifest:
        ...
        samples=pose_rows(ego); first=samples[0][1]; rebase.append({"clip_id":cid,"formula":"inverse(T_world_world_global) @ T_rig_world_raw","T_world_world_global":first,"first_pose_identity":all(abs(first[i][j]-(1.0 if i==j else 0.0))<1e-9 for i in range(4) for j in range(4)),"source":NCORE_URL,"rebase_verified":True})
        ...
        chosen=vs["B_REBASED_EGO"]; summaries.append({"clip_id":cid,**chosen,"per_clip_offset_rederived":False});
        for r,t,raw,b,ts in rows:
            c=center(b); q=quat(b); all_cross.append({"clip_id":cid,"track_id":str(r["track_id"]),"pai_timestamp_us":int(r["timestamp_us"]),"nurec_timestamp_us":ts,"center_x":c[0],"center_y":c[1],"center_z":c[2],"quaternion_x":q[0],"quaternion_y":q[1],"quaternion_z":q[2],"quaternion_w":q[3],"center_residual_m":math.dist(c,t["center"]),"yaw_residual_deg":math.degrees(abs(_wrap(pose_yaw(_pose(c,q))-pose_yaw(_pose(t["center"],t["quaternion"]))))),"dimension_max_abs_m":max(abs(float(r[k])-float(t["dimensions"][i])) for i,k in enumerate(("size_x","size_y","size_z"))),"track_id_match":"EXACT_ID_PRESERVED","transform":"T_world_ref(UNRESOLVED->identity diagnostic) @ T_rig_world_local @ T_object_rig"})
    ...
    _dump(out/"local_nurec_world_ref_contract.json",{"status":"UNRESOLVED_LOCAL_SERIALIZATION_EDGE","clips":worldrefs,"no_fitted_transform":True})
    _dump(out/"pai_to_nurec_exact_transform_contract.json",{"status":"PARTIALLY_VERIFIED","formula":"T_world_ref @ T_rig_world_local @ T_object_rig","world_ref_status":"UNRESOLVED_LOCAL_SERIALIZATION_EDGE","timestamp_mapping":"existing per-clip offset reused","no_fitted_transform":True})
    _dump(out/"pai_nurec_exact_crosscheck_summary.json",{"clips":summaries,"matched_observation_count":len(all_cross),"world_ref_status":"UNRESOLVED_LOCAL_SERIALIZATION_EDGE"})
