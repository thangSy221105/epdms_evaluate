...
def main():
    ...
    for item in manifest:
        ...
        mapped_ts = sorted({int(r["timestamp_us"]) + offset for r in pai_rows})
        cf_queries, ttc_queries = build_queries(time_records[cid]["nurec_t0_us"])
        def recheck(queries, tolerance, target):
            for query in queries:
                nearest = min(mapped_ts, key=lambda ts: abs(ts - query), default=None)
                delta = abs(nearest - query) if nearest is not None else None
                within = nearest is not None and delta <= tolerance
                target.append({"clip_id": cid, "query_timestamp_us": query, "nearest_annotation_timestamp_us": nearest if within else None, "delta_us": delta, "object_count": sum(1 for ts in mapped_ts if within and ts == nearest), "frame_annotation_status": "OBJECT_ROWS_PRESENT_BUT_COMPLETENESS_UNKNOWN" if within else "NO_OBJECT_ROWS_COMPLETENESS_UNKNOWN", "authoritative_frame_coverage": False, "complete_observation": False, "query_ready": False})
